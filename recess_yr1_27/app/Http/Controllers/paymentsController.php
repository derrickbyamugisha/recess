    ...
    public function show()
		{ 	$ddata = new donations;
		    ...
		    if($don_total > 2000000 ){ $don_for_share= $don_total-2000000; $amount_for_treasury=2000000; }else{$amount_for_treasury =$don_total; }
		     if($wage_units_total!=0){
		     	$amont_per_unit = $don_for_share/$wage_units_total; 
		     	$affectdrs = DB::table('agents')->update(['wage_per_unit'=>$amont_per_unit]);
		     }else{
		     	$amont_per_unit =0;
		     	$affectdrs = DB::table('agents')->update(['wage_per_unit'=>$amont_per_unit]);
		     }
		    ...
		    Storage::disk('local')->put('members.txt', '');
		    Storage::disk('local')->put('status.txt', ''); 
		    Storage::disk('local')->put('signitures.txt', ''); 
